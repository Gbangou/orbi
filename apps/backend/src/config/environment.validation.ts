type EnvironmentVariables = {
  ...
  PAYMENT_ATTEMPT_RECONCILIATION_SWEEP_INTERVAL_MS?: string;
  PAYMENT_ATTEMPT_RECONCILIATION_STALE_AFTER_MS?: string;
  PAYMENT_ATTEMPT_RECONCILIATION_BATCH_SIZE?: string;
...
export function validateEnvironment(config: EnvironmentVariables) {
  ...
  return {
    ...
    PAYMENT_ATTEMPT_RECONCILIATION_SWEEP_INTERVAL_MS:
      config.PAYMENT_ATTEMPT_RECONCILIATION_SWEEP_INTERVAL_MS ?? '120000',
    PAYMENT_ATTEMPT_RECONCILIATION_STALE_AFTER_MS:
      config.PAYMENT_ATTEMPT_RECONCILIATION_STALE_AFTER_MS ?? '600000',
    PAYMENT_ATTEMPT_RECONCILIATION_BATCH_SIZE:
      config.PAYMENT_ATTEMPT_RECONCILIATION_BATCH_SIZE ?? '25',
